(function($) {
		...
		var jqXHR=$.ajax({
			...
			statusCode: {
				...
				400: function(response) {
					...
					// Process error
					$.WS_Form.this.api_call_error_handler(response, 400, url, error_callback);
				},
				...
				401: function(response) {
					...
					// Process error
					$.WS_Form.this.api_call_error_handler(response, 401, url, error_callback);
				},
				...
				403: function(response) {
					...
					// Process error
					$.WS_Form.this.api_call_error_handler(response, 403, url, error_callback);
				},
				...
				404: function(response) {
					...
					// Process error
					$.WS_Form.this.api_call_error_handler(response, 404, url, error_callback);
				},
				...
				500: function(response) {
					...
					// Process error
					$.WS_Form.this.api_call_error_handler(response, 500, url, error_callback);
				}
			}
		});
	...
	$.WS_Form.prototype.field_render = function(obj, field_data) {
		...
		if(field_type_has_mask_preview) {
			...
			// WPAutoP
			var meta_wpautop = ((typeof(field_type.meta_wpautop) !== 'undefined') ? field_type.meta_wpautop : false);
			if(meta_wpautop) { field_preview = this.wpautop(field_preview); }
...
//					}
	...
	$.WS_Form.prototype.sidebar_option_action_process_render = function(options_action_obj, options_action, meta_value, complete) {
		...
		for(var options_action_index in options_action) {
			...
			options_html += '<option value="' + this.html_encode(option_action_single.value) + '"' + ((meta_value.toString() === option_action_single.value.toString()) ? ' selected="selected"' : '') + '>' + this.html_encode(option_action_single.text) + '</option>';
		}
	...
	$.WS_Form.prototype.sidebar_repeater_row_html = function(repeater, repeater_data_row, repeater_meta_key) {
		...
		repeater_row_html += '<td data-icon><div data-action="wsf-repeater-row-delete"' + this.tooltip(this.language('repeater_row_delete'), 'top-right') + '>' + this.svg('minus-circle') + '</div></td>';
		...
		for(var template_category_index in $.WS_Form.templates_section) {
			...
			for(var template_index in templates) {
				...
				if(template_category_delete) {
					...
					section_select_html_templates += '<div data-action="wsf-section-delete"' + this.tooltip(this.language('section_delete'), 'top-right') + '>' + this.svg('minus-circle') + '</div>';
				}
	...
	$.WS_Form.prototype.data_grid_html = function(meta_key, meta_value, data_source_id, read_only) {
		...
		for(var group_index in groups) {
			...
			if(!read_only && group_count > 1) {
				return_html += '<div data-action="wsf-data-grid-group-delete"' + this.tooltip(this.language('data_grid_group_delete'), 'top-center') + '>' + this.svg('close-circle') + '</div>';
			}
	...
	$.WS_Form.prototype.data_grid_html_row_header = function(columns, group_index, row_default, row_disabled, row_required, row_hidden, meta_key, read_only) {
		...
		for(var key in columns) {
			...
			if(!read_only && (columns.length > 1) && ((max_columns > 1) || (max_columns == 0))) {
				...
				return_html += '<div data-action="wsf-data-grid-column-delete" data-id="' + column_id + '"' + this.tooltip(this.language('data_grid_column_delete'), 'bottom-center') + '>' + this.svg('close-circle') + '</div>';
			}
	...
	$.WS_Form.prototype.data_grid_html_row = function(row, group_index, row_index, row_default, row_disabled, row_required, row_hidden, meta_key, read_only) {
		...
		if(!read_only) {
			...
			return_html += '<td data-fixed-icon><div data-action="wsf-data-grid-row-delete"' + this.tooltip(this.language('data_grid_row_delete'), 'top-right') + '>' + this.svg('minus-circle') + '</div></td>';
		}
		...
		$('[data-action]:not([data-action="wsf-clipboard"])').on('click', function() {
			...
			switch(action) {
				...
				case 'wsf-export-all' :
					...
					$('#ws-form-date-from').val($('#wsf_filter_date_from').val());
					$('#ws-form-date-to').val($('#wsf_filter_date_to').val());
			...
			for(var section_index in group.sections) {
				...
				for(var section_repeatable_array_index in section_repeatable_array) {
					...
					for(var field_index in section.fields) {
						...
						var submit_edit_ecommerce_field = (typeof(field_type.submit_edit_ecommerce) !== 'undefined') ? field_type.submit_edit_ecommerce : false;
