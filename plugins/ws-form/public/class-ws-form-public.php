...
//			self::wp_scripts_debug();
		...
		public function enqueue() {
			...
			if(!$this->enqueued_js_form_common && apply_filters('wsf_enqueue_js_form_common', $this->enqueue_js_form_common)) {
				...
				wp_register_script($this->plugin_name . '-form-common', sprintf('%sshared/js/ws-form%s.js', WS_FORM_PLUGIN_DIR_URL, $min), array('jquery'), $this->version, $jquery_footer);
				...
				if(WS_Form_Common::option_get('css_layout', true) && (WS_Form_Common::option_get('framework', 'ws-form') == 'ws-form')) {
					...
					if($this->customizer || ($this->css_inline && !is_admin())) {
						...
						add_action('wp_footer', function() {
							...
							// Output public CSS
							$css = $this->ws_form_css->get_public(null, $this->customizer);
						...
						wp_enqueue_style($this->plugin_name . '-layout', $css_compile ? sprintf('%s/ws-form/%s/public.layout%s.css', wp_upload_dir()['baseurl'], WS_FORM_CSS_FILE_PATH, $min) : WS_Form_Common::get_api_path('helper/ws-form-css'), array(), $this->version, 'all');
					}
				}
				...
				if(WS_Form_Common::option_get('css_skin', true) && (WS_Form_Common::option_get('framework', 'ws-form') == 'ws-form')) {
					...
					if($this->customizer || ($this->css_inline && !is_admin())) {
						...
						add_action('wp_footer', function() {
							...
							// Output public CSS
							$css_skin = $this->ws_form_css->get_skin(null, $this->customizer);
						...
						wp_enqueue_style($this->plugin_name . '-skin', $css_compile ? sprintf('%s/ws-form/%s/public.skin%s.css', wp_upload_dir()['baseurl'], WS_FORM_CSS_FILE_PATH, $min) : WS_Form_Common::get_api_path('helper/ws-form-css-skin'), array(), $this->version, 'all');
					}
				}
