<?php
	...
	class WS_Form_Common {
									...
									switch($parse_variable) {
										...
										case 'field_float' :
											...
											if(isset($submit->meta[$meta_key])) {
												...
												$field_static = isset($field_config['static']) ? $field_config['static'] : false;
												...
												if($field_static) {
													...
													if($field_static === true) {
														...
														$value = isset($field_config['mask_field_static']) ? $field_config['mask_field_static'] : '';
		...
		public static function parse_variables_meta_value($form, $meta, $content_type, $column = 'parse_variable', $column_id_default = 0, $datagrid_delimiter = NULL) {
		...
		// #email_submission
		public static function parse_variables_fields_all($form, $submit, $content_type, $render_group_labels, $render_section_labels, $render_field_labels, $render_blank_fields, $render_static_fields, $hidden_array) {

			$fields_all = self::parse_variables_fields_all_group($form->groups, $form, $submit, $content_type, $render_group_labels, $render_section_labels, $render_field_labels, $render_blank_fields, $render_static_fields, $hidden_array);

			return $fields_all;
		}

		// Run through each group
		public static function parse_variables_fields_all_group($groups, $form, $submit, $content_type, $render_group_labels, $render_section_labels, $render_field_labels, $render_blank_fields, $render_static_fields, $hidden_array) {
			...
			foreach($groups as $key => $group) {
				...
				if(isset($groups[$key]->sections)) {
					...
					$sections_html = self::parse_variables_fields_all_section($group->sections, $form, $submit, $content_type, $render_section_labels, $render_field_labels, $render_blank_fields, $render_static_fields, $hidden_array);
		...
		// Run through each section
		public static function parse_variables_fields_all_section($sections, $form, $submit, $content_type, $render_section_labels, $render_field_labels, $render_blank_fields, $render_static_fields, $hidden_array) {
					...
					foreach($section->fields as $field) {
						...
						if($render_static_fields && $field_static) {
							...
							if($field_static === true) {
								...
								// If static set to true, we use the mask_field
								$mask_field = isset($field_type['mask_field_static']) ? $field_type['mask_field_static'] : '';
								...
								// WPAutoP
								$meta_wpautop = isset($field_type['meta_wpautop']) ? $field_type['meta_wpautop'] : false;					
		...
		public static function get_datagrid_value($field, $value_array, $content_type, $default_value, $column = 'parse_variable', $column_id_default = 0, $datagrid_delimiter = NULL) {
			...
			foreach($groups as $group) {
				...
				foreach($rows as $row) {
					...
					if(in_array($data[$value_column_index], $value_array)) {
