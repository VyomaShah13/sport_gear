(function($) {
	...
	$.WS_Form.prototype.get_field_html = function(field, section_repeatable_index) {
		...
		if(label_render) {
			...
			// Check label position is valid for this framework
			...
			if(label_position_inside) {
				...
				var label_position_inside_class = typeof(framework['label_position_inside_class'] !== 'undefined') ? framework['label_position_inside_class'] : false;

				if(label_position_inside_class !== false) {

					class_array.push(label_position_inside_class);
				}
			}
		}
...
//		}
	...
	$.WS_Form.prototype.get_field_html_single = function (field, value, is_submit, section_repeatable_index) {
		...
		if(invalid_feedback_render) {
			...
			if(invalid_feedback == '' && this.invalid_feedback_mask_placeholder != '') {
				...
				var invalid_feedback = this.replace_all(this.invalid_feedback_mask_placeholder, '#label_lowercase', invalid_feedback_label.toLowerCase());
		...
		if(label_position_inside) { label_position = 'bottom'; }
	...
	$.WS_Form.prototype.set_invalid_feedback = function(obj, invalid_feedback_obj, value, object_id, object_row_id) {
		...
		if(obj.length && obj[0].willValidate) {
			...
			obj[0].setCustomValidity(value);
			if(value !== '') {

				this.validation_message_cache[object_id] = value;

			} else {

				delete this.validation_message_cache[object_id];
			}
		}
