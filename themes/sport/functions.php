...
if ( ! function_exists( 'sport_setup' ) ) :
	...
	function sport_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'sport' ),
			)
		);
...
function sport_scripts() {
	...
	wp_style_add_data( 'sport-style', 'rtl', 'replace' );

	wp_enqueue_script( 'sport-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
